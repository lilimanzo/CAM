...
subroutine generate_stochastic_clouds( &
   ...
   real(kind=r8), intent(in) :: tauc(:,:,:)        ! cloud optical depth
                                                     !    Dimensions: (nbndlw,ncol,nlay)
   real(kind=r8), intent(in) :: liq_tauc(:,:,:)    ! XL cloud liq optical depth
   real(kind=r8), intent(in) :: ice_tauc(:,:,:)    ! XL cloud ice optical depth                                                  
   ...
   real(kind=r8), intent(out) :: tauc_stoch(:,:,:) ! subcolumn cloud optical depth
                                                     !    Dimensions: (ngptlw,ncol,nlay)
   real(kind=r8), intent(out) :: liq_tauc_stoch(:,:,:) ! XL subcolumn cloud liq optical depth
   real(kind=r8), intent(out) :: ice_tauc_stoch(:,:,:) ! XL subcolumn cloud ice optical depth
      ...
      do i = 1, ncol
         do isubcol = 1,nsubcol
            if ( iscloudy(isubcol,i,ilev) .and. (cldf(i,ilev) > 0._r8) ) then
               ...
               ice_tauc_stoch(isubcol,i,ilev) = ice_tauc(n,i,ilev) ! is consistent with the total???
            else
               tauc_stoch(isubcol,i,ilev) = 0._r8
               liq_tauc_stoch(isubcol,i,ilev) = 0._r8 ! XL
               ice_tauc_stoch(isubcol,i,ilev) = 0._r8 ! XL
            end if
         end do
      end do
   end do
