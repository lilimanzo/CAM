...
! XL additional diagnostics qrl_diag, flnt_diag, flut_diag, flutc_diag, flns_diag, flnsc_diag, flwds_diag
...
subroutine radiation_init(pbuf2d)
   ...
   if (is_first_step()) then
      call pbuf_set_field(pbuf2d, qrl_idx, 0._r8) ! XL: add qrl_diag_idx here as well??
   end if
...
subroutine radiation_tend( &
   ...
   if (dosw .or. dolw) then
      ...
      if (dolw) then
         ...
         do icall = N_DIAG, 0, -1
            ...
            if (active_calls(icall)) then
               ...
               ! liq_lw_abs, ice_lw_abs, coszrs, cam_in%landfrac, cam_in%icefrac, clat, clon, state
               call rad_rrtmg_lw( &
                  lchnk, ncol, num_rrtmg_levs, r_state, state%pmid,  &
      ...
      do k =1 , pver
         do i = 1, ncol
            qrs(i,k) = qrs(i,k)/state%pdel(i,k) ! XL: note this is not executed as long as if (dosw .or. dolw)
